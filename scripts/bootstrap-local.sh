#!/usr/bin/env bash
# Create the local-only MySQL database and ignored runtime configuration.
...
readonly DB_NAME="movieworld_telegram_dev"
readonly DB_USER="movieworld_telegram_dev"
...
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE ON \`${DB_NAME}\`.* TO '${DB_USER}'@'127.0.0.1';
FLUSH PRIVILEGES;
SQL
...
echo "Local setup complete. Run ./scripts/run-local.sh, then ./scripts/test-local.sh in another terminal."
